...
import { CreateExerciseRecordDto, UpdateExerciseRecordDto } from "@/shared/models"; 
import { throwAnyErrors } from "@/shared/lib";

// Get all exercise records
export const getAllExerciseRecords = async (): Promise<CreateExerciseRecordDto[]> => {
  return await throwAnyErrors(GET('/sport/exercise-records'));
};

// Create a new exercise record
export const createExerciseRecord = async (body: CreateExerciseRecordDto): Promise<CreateExerciseRecordDto> => {
  return await throwAnyErrors(POST('/sport/exercise-records', { body }));
};

// Update an existing exercise record
export const updateExerciseRecord = async (id: number, body: UpdateExerciseRecordDto): Promise<UpdateExerciseRecordDto> => {
